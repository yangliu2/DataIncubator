...
def scrape_zillow(selected_address):
    ...
    for i in range(len(selected_address)):
        ...
        if pd.isnull(address.iloc[index].ZILLOW_STATUS) and pd.isnull(address.iloc[index].ZILLOW_ID):
...
def selectHouse(budget, built, cusine, food_section, grade):
    ...
    for i in range(len(latitudes)):
        if i < len(latitudes) - 1:
            script += "[ '"+ links_sale['ZILLOW_ID'].iloc[i] + "', " + str(latitudes[i]) + ", " + str(longitudes[i]) + ", '" + links_sale['ZILLOW_LINK'].iloc[i] + "' ],"
        else:
            script += "[ '"+ links_sale['ZILLOW_ID'].iloc[i] + "', " + str(latitudes[i]) + ", " + str(longitudes[i]) + ", '" + links_sale['ZILLOW_LINK'].iloc[i] + "' ]"
    #close bracket
    script += "];"
